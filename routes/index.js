...
router.post('/payment-create', async (req, res) => {
  ...
  const auth = Buffer
    .from("rzp_live_Yfnp0JSMDmIFvv:WuBf9SuLkWzXyENB29QTbUw0")
    .toString("base64");
...
router.post('/payment-success', (req, res) => {
  ...
  pool.query(
    ...
    (err, cartItems) => {
      ...
      pool.query(
        `INSERT INTO booking
        (usernumber, booking_id, quantity, price, payment_mode, status, orderid, order_date,
         categoryid, oneprice, size, net_amount, product_name, address)
        VALUES ?`,
        [values]
      );
      ...
      pool.query(
        `INSERT INTO transaction (type, amount, orderid, date)
         VALUES ('ONLINE', ?, ?, ?)`,
        [cartItems.reduce((s, i) => s + i.price, 0), orderId, date]
      );
...
router.post('/place-order-cod', (req, res) => {
  ...
  pool.query(
    ...
    (err, cart) => {
      ...
      pool.query(
        `INSERT INTO transaction (type, amount, orderid, date)
         VALUES ('COD', ?, ?, ?)`,
        [cart.reduce((s, i) => s + i.price, 0), orderId, date]
      );
...
router.post('/verify-payment', async (req, res) => {
  try {
    ...
    const cart = await query(
      `SELECT * FROM cart WHERE usernumber = ?`,
      [number]
    );
    ...
    for (const i of cart) {
      await query(
        `INSERT INTO booking
        (name, number, address, price, status, usernumber, payment_mode,
         orderid, order_date, quantity, booking_id, net_amount)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
        [
          name,
          number,
    ...
    await query(
      `INSERT INTO transaction (type, amount, orderid, date)
       VALUES ('ONLINE', ?, ?, ?)`,
      [amount, orderId, new Date().toISOString().slice(0, 10)]
    );

    await query(`DELETE FROM cart WHERE usernumber = ?`, [number]);
