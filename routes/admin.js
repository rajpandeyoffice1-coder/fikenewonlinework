...
router.get('/all', (req, res) => {
    pool.query(`SELECT * FROM shipping ORDER BY id DESC`, (err, rows) => {
        if (err) {
            console.error(err)
...
function extractFromBooking(b) {
    ...
    if (!pincode) errors.push('pincode missing')

    const nameParts = (b.name || '').trim().split(' ').filter(Boolean)
    if (!nameParts.length) errors.push('customer name missing')

    const firstName = nameParts[0]
    const lastName = nameParts.slice(1).join(' ') || 'NA'

    const rawProduct = b.product_name
    if (!rawProduct) errors.push('product name missing')

    const productName = rawProduct
        ? rawProduct.replace(/["']/g, '').trim().substring(0, 100)
        : null
    ...
    const city = parts.length >= 2 ? parts[parts.length - 2] : 'Delhi'
    const state = parts.length >= 1 ? parts[parts.length - 1] : 'Delhi'
    ...
    console.log('=========== SHIPROCKET REQUIRED CHECK ===========')
    console.log({
        billing_customer_name: !!firstName,
        billing_last_name: true,
        billing_phone: !!b.number,
    ...
    if (errors.length) {
        console.log('❌ SHIPROCKET BLOCKED — MISSING DATA')
        console.log(errors)
...
function getStatusEmailTemplate({ username, orderId, status, awb, totalAmount }) {
    let message = ''

    switch (status) {
        case 'Order Accepted':
            message = `<p>Your order has been accepted and is being prepared.</p>`
            break
        case 'Dispatched':
            message = `<p>Your order has been dispatched.</p><p><strong>AWB:</strong> ${awb}</p>`
            break
        case 'Delivered':
            message = `<p>Your order has been delivered successfully.</p>`
            break
        case 'Cancel':
            message = `<p>Your order has been cancelled.</p>`
            break
        default:
            message = `<p>Your order status is updated to <strong>${status}</strong>.</p>`
    }
    ...
    return `
        ...
        ${message}
        <p>For support contact <a href="mailto:info@fikaonline.in">info@fikaonline.in</a></p>
        <p>Thank you for shopping with FIKA</p>
        <p><strong>FIKA Team</strong></p>
        <a href="https://fikaonline.in">https://fikaonline.in</a>
    `
}
...
router.get('/update-status', async (req, res) => {
    try {
        ...
        if (status === 'Order Accepted') {
            ...
            pool.query(`SELECT * FROM booking WHERE id = ?`, [id], async (err, result) => {
                ...
                let token
                try {
                    token = await deliveryApi.shippingAuthLogin()
                } catch {
                    return res.status(400).json({ error: 'Shiprocket auth failed' })
                }
                ...
                const orderDetails = {
                    ...
                    billing_customer_name: parsed.firstName,
                    billing_last_name: parsed.lastName,
                    ...
                    payment_method: b.payment_mode,
                    order_items: [{
                        name: parsed.productName,
                        sku: b.product_sku || `SKU-${b.id}`,
                        units: 1,
                        selling_price: parsed.price
                    }],
                    sub_total: parsed.price,
                    length: 10,
                    breadth: 10,
                    height: 10,
                    weight: 1
                ...
                try {
                    const shipRes = await deliveryApi.createShippingOrder(orderDetails, token)

                    pool.query(
                        `INSERT IGNORE INTO shipping (order_id, channel_order_id, shipment_id, status)
                 VALUES (?,?,?,?)`,
                        [shipRes.order_id, b.orderid, shipRes.shipment_id, 'Created']
                    )

                    pool.query(`UPDATE booking SET status=? WHERE id=?`, ['Order Accepted', id])

                    pool.query(
                        `INSERT INTO alert (usernumber, status, orderid, date)
                 VALUES (?,?,?,?)`,
                        [b.usernumber, 'Order Accepted', b.orderid, today]
                    )

                    res.json({ success: true })

                } catch (e) {
                    res.status(400).json(e.response?.data || { error: 'Shiprocket rejected order' })
                }
            })
        ...
        pool.query(`SELECT * FROM booking WHERE id=?`, [id], async (err, result) => {
            ...
            pool.query(
                `INSERT INTO alert (usernumber,status,orderid,date) VALUES (?,?,?,?)`,
                [b.usernumber, status, b.orderid, today],
                async (err) => {
                    if (err) return res.status(500).json({ error: 'Alert insert failed' })
